    ...
    run: async (supabase) => {
      ...
      const payload = rows.map((row) => {
        ...
        return {
          ...
          status,
          payload_json: (row.payload || {}) as Json,
          resposta_json: {
          ...
          tentativas: Number(row.tentativas || 0),
          recebido_em:
            row.ultimo_recebido_em || row.primeiro_recebido_em || timestamp,
      ...
      return {
        total: rows.length,
        erros: rows.filter(
          (row) => buildWebhookStatus(row.status_processamento) === "erro"
        ).length,
        processados: rows.filter(
          (row) =>
            buildWebhookStatus(row.status_processamento) === "processado"
        ).length,
