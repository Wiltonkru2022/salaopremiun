    ...
    .upsert(
      {
        ...
        titulo: sanitizeText(params.titulo, "SalãoPremium"),
  ...
  await Promise.all([
    agendamento.profissional_id
      ? queueNotificationJob({
          ...
          mensagem: `Olá, ${profissionalNome}. Seu atendimento com ${clienteNome} (${servicoNome}) começa em ${minutosAntes} minutos, ${quando}.`,
    ...
    clienteAppContaId
      ? queueNotificationJob({
          ...
          mensagem: `Cuidado com atrasos: seu atendimento é daqui ${minutosAntes} minutos. Organize-se para não atrasar o profissional ${profissionalNome}.`,
...
export async function notifyAppointmentRescheduled(params: {
  ...
  await Promise.all([
    ...
    queueNotificationJob({
      ...
      titulo: params.actor === "cliente" ? "Cliente reagendou pelo app" : "Horário reagendado",
  ...
  await Promise.all([
    ...
    params.idProfissional
      ? queueNotificationJob({
          ...
          titulo: "Horário cancelado",
          mensagem: `${clienteNome} cancelou ${servicoNome} de ${quando}. O horário foi liberado.`,
    ...
    queueNotificationJob({
      ...
      titulo: params.actor === "cliente" ? "Cliente cancelou pelo app" : "Horário cancelado",
      mensagem: `${clienteNome} cancelou ${servicoNome} de ${quando}. O horário ficou livre na agenda.`,
      url: "/agenda",
  ...
  await Promise.all([
    ...
    clienteAppContaId
      ? queueNotificationJob({
          ...
          titulo: "Atendimento finalizado",
          mensagem: `Atendimento finalizado. Obrigado pela preferência, volte sempre.${valorLabel}`,
    ...
    clienteAppContaId
      ? queueNotificationJob({
          ...
          mensagem: `Como foi seu atendimento com ${profissionalNome}? Sua avaliação ajuda o salão a cuidar melhor de você.`,
          url: `/app-cliente/agendamentos/${agendamento.id}/avaliar`,
...
export async function notifyReviewReceived(params: {
  ...
  await queueNotificationJob({
    ...
    canal: "salao_painel",
    tipo: tone,
    titulo: params.nota <= 3 ? "Avaliação precisa de atenção" : "Nova avaliação recebida",
    mensagem:
      params.nota <= 3
        ? `Cliente avaliou com ${params.nota} estrela(s). Recomendado entrar em contato.`
        : `Nova avaliação ${params.nota} estrela(s) recebida no app cliente.`,
