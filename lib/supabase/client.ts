...
async function remoteRequest(body: unknown) {
  try {
    ...
    return { data: null, error: { message: `Falha HTTP ${response.status} ao acessar o Neon.` }, count: null, status: response.status, statusText: response.statusText };
  } catch (cause) {
    return { data: null, error: { message: cause instanceof Error ? cause.message : "Falha de rede ao acessar o Neon." }, count: null, status: 0, statusText: "Network Error" };
  }
}
...
function createRemoteBuilder(table: string) {
  ...
  const builder: any = {
    select(columns = "*", options?: Record<string, unknown>) { state.select = columns; state.selectOptions = options; return builder; },
    ...
    not(column: string, operator: string, value: unknown) { state.filters.push({ op: `not.${String(operator || "").toLowerCase()}`, column, value }); return builder; },
    filter(column: string, operator: string, value: unknown) { state.filters.push({ op: String(operator || "").toLowerCase(), column, value }); return builder; },
    or(value: string) { state.filters.push({ op: "or", value }); return builder; },
    match(values: Record<string, unknown>) { Object.entries(values || {}).forEach(([column, value]) => state.filters.push({ op: "eq", column, value })); return builder; },
    ...
    async then(resolve: (value: any) => unknown, reject?: (reason: unknown) => unknown) {
      try {
        ...
        if (throwOnError && result?.error) throw Object.assign(new Error(result.error.message || "Erro na consulta Neon."), result.error);
...
async function readSession() {
  const response = await fetch("/api/painel/session", { method: "GET", cache: "no-store", credentials: "same-origin" });
...
function createAuthProxy() {
  return {
    async getUser() {
      try { return { data: { user: await readSession() }, error: null }; }
      catch (cause) { return { data: { user: null }, error: { message: cause instanceof Error ? cause.message : "Falha ao validar sessao Clerk." } }; }
    },
    async getSession() {
      try {
        const user = await readSession();
        return { data: { session: user ? { access_token: "painel-clerk-session", refresh_token: "", token_type: "bearer", expires_in: 0, expires_at: 0, user } : null }, error: null };
      } catch (cause) {
        return { data: { session: null }, error: { message: cause instanceof Error ? cause.message : "Falha ao validar sessao Clerk." } };
      }
    },
    async signOut() {
      const response = await fetch("/api/auth/painel/logout", { method: "POST", cache: "no-store", credentials: "same-origin" });
      return response.ok ? { error: null } : { error: { message: "Nao foi possivel encerrar a sessao Clerk." } };
    },
    onAuthStateChange() { return { data: { subscription: { unsubscribe() {} } } }; },
    async signInWithPassword() { return { data: { session: null, user: null }, error: { message: "Login por senha local foi encerrado. Use o login Clerk do SalaoPremium." } }; },
  };
}
...
function createBackendClient() {
  ...
  return new Proxy({} as any, {
    get(_target, property) {
      ...
      if (property === "rpc") return async (fn: string, args?: Record<string, unknown>) => remoteRequest({ kind: "rpc", fn, args: args || {} });
...
/** Nome mantido para compatibilidade; nenhuma conexao Supabase e criada no browser. */
export function createClient() {
  if (!browserClient) browserClient = createBackendClient();
