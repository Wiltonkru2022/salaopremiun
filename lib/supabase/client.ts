...
async function remoteRequest(body: unknown) {
  try {
    ...
    return {
      data: null,
      error: { message: cause instanceof Error ? cause.message : "Falha de rede ao acessar dados do painel." },
      count: null,
      status: 0,
  ...
  const builder: any = {
    ...
    async then(resolve: (value: any) => unknown, reject?: (reason: unknown) => unknown) {
      try {
        ...
        if (throwOnError && result?.error) throw Object.assign(new Error(result.error.message || "Erro na consulta."), result.error);
        return resolve(result);
      } catch (cause) { if (reject) return reject(cause); throw cause; }
    },
  };
  return builder;
}
...
function createAuthProxy() {
  return new Proxy({} as any, {
    get(_target, property) {
      if (property === "getUser") return async () => {
        try {
          ...
          return response.ok && payload?.user?.id ? { data: { user: payload.user }, error: null } : { data: { user: null }, error: null };
        } catch (cause) { return { data: { user: null }, error: { message: cause instanceof Error ? cause.message : "Falha ao validar sessao." } }; }
      };
      if (property === "getSession") return async () => {
        const result = await (createAuthProxy() as any).getUser();
        return result.data.user ? { data: { session: { access_token: "clerk-session", user: result.data.user } }, error: null } : { data: { session: null }, error: result.error || null };
      };
      if (property === "signOut") return async () => {
        try {
          const response = await fetch("/api/auth/painel/logout", { method: "POST", cache: "no-store", credentials: "same-origin" });
          return response.ok ? { error: null } : { error: { message: "Nao foi possivel encerrar a sessao." } };
        } catch (cause) { return { error: { message: cause instanceof Error ? cause.message : "Falha ao sair." } }; }
      };
      if (property === "onAuthStateChange") return () => ({ data: { subscription: { unsubscribe() {} } } });
      return () => Promise.resolve({ data: null, error: { message: `Metodo de autenticacao legado '${String(property)}' foi removido. Use Clerk ou a API interna.` } });
    },
  });
}

function createNoopRealtimeChannel() {
  const channel: any = { on() { return channel; }, subscribe(callback?: (status: string) => void) { callback?.("SUBSCRIBED"); return channel; }, unsubscribe: async () => "ok" };
  return channel;
}
...
/** Nome mantido para compatibilidade de imports antigos. Banco: Neon via API. Auth: Clerk. */
