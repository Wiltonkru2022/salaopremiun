...
function readJsonPath(row: Record<string, unknown>, column: string) {
  ...
  for (const part of parts.slice(1)) {
    if (!SIMPLE_COLUMN.test(part) || !current || typeof current !== "object") {
      return undefined;
    }
...
function compareFilter(actual: unknown, operator: string, expected: unknown) {
  switch (operator) {
    ...
    case "is":
      ...
      return false;
    case "like":
      return compareLike(actual, expected, false);
    case "ilike":
      return compareLike(actual, expected, true);
    case "in":
    ...
    case "contains": {
      ...
      if (actual && expected && typeof actual === "object" && typeof expected === "object") {
        return Object.entries(expected as Record<string, unknown>).every(
          ([key, value]) =>
            String((actual as Record<string, unknown>)[key] ?? "") === String(value ?? "")
        );
      }
      return false;
    }
    default:
      throw new Error(`Filtro de compatibilidade nao suportado: ${operator}`);
  }
}
...
function applyPostFilters(rows: unknown[], filters: PostFilter[]) {
  ...
  return rows.filter((item) => {
    ...
    return filters.every((filter) => {
      const matched = compareFilter(
        readJsonPath(row, filter.column),
        filter.operator,
        filter.value
      );
...
function createBuilder(_legacyTarget: SupabaseLike, table: string) {
  ...
  const builder: any = {
    ...
    eq(column: string, value: unknown) {
      return addFilter("eq", column, value);
    },
    neq(column: string, value: unknown) {
      return addFilter("neq", column, value);
    },
    gt(column: string, value: unknown) {
      return addFilter("gt", column, value);
    },
    gte(column: string, value: unknown) {
      return addFilter("gte", column, value);
    },
    lt(column: string, value: unknown) {
      return addFilter("lt", column, value);
    },
    lte(column: string, value: unknown) {
      return addFilter("lte", column, value);
    },
    like(column: string, value: unknown) {
      return addFilter("like", column, value);
    },
    ilike(column: string, value: unknown) {
      return addFilter("ilike", column, value);
    },
    is(column: string, value: unknown) {
      return addFilter("is", column, value);
    },
    in(column: string, value: unknown[]) {
      return addFilter("in", column, value);
    },
    contains(column: string, value: unknown) {
      return addFilter("contains", column, value);
    },
    not(column: string, operator: string, value: unknown) {
      const inverse = SIMPLE_COLUMN.test(column) ? inverseOperator(operator) : null;
      if (inverse) return addFilter(inverse, column, value);
      state.postFilters.push({ column, operator, value, negate: true });
      return builder;
    },
    filter(column: string, operator: string, value: unknown) {
      const normalized = String(operator || "").trim().toLowerCase();
      if (normalized.startsWith("not.")) {
        return builder.not(column, normalized.slice(4), value);
      }
      ...
      if (!allowed.has(normalized)) {
        throw new Error(`Operador filter nao suportado no Neon: ${operator}`);
      }
    ...
    async then(
      ...
      try {
        if (state.mutation && state.postFilters.length) {
          throw new Error(
            "Mutacao com filtro JSON/NOT complexo ainda nao e permitida no adaptador Neon."
          );
        }
        ...
        const queryForNeon: PainelDbQuery = {
          ...
          ...(hasPostFilters
            ? {
                ...
                single: false,
                maybeSingle: false,
                selectOptions: state.selectOptions?.head
                  ? { ...state.selectOptions, head: false }
                  : state.selectOptions,
              }
            : {}),
        };
        ...
        if (hasPostFilters && !result?.error) {
          ...
          if (state.range) {
            ...
            rows = rows.slice(0, Math.max(0, Number(state.limit) || 0));
          }
          ...
          if (state.selectOptions?.head) {
            result = {
              ...result,
              data: null,
              count: state.selectOptions.count === "exact" ? total : result.count,
            };
            ...
            if (state.single && rows.length !== 1) {
              result = {
                ...
                data: null,
                error: {
                  message: `Esperado 1 registro; encontrados ${rows.length}.`,
                  code: "PGRST116",
                },
                status: 406,
                statusText: "Not Acceptable",
              };
            } else if (state.maybeSingle && rows.length > 1) {
              result = {
                ...result,
                data: null,
                error: {
                  message: `Esperado no maximo 1 registro; encontrados ${rows.length}.`,
                  code: "PGRST116",
                },
                status: 406,
                statusText: "Not Acceptable",
...
export function createNeonSupabaseCompat(legacyTarget: SupabaseLike = {}): SupabaseLike {
  return new Proxy(legacyTarget || {}, {
    get(target, property, receiver) {
      if (property === "from") {
        return (table: string) => createBuilder(legacyTarget, table);
      }
      if (property === "rpc") {
        return async (fn: string, args?: Record<string, unknown>) => executeNeonRpc(fn, args);
      }
