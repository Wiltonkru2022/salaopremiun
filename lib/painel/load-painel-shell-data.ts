...
const loadPainelShellContextCached = unstable_cache(
  async (authUserId: string) => {
    ...
    if (usuarioError) return { ok: false as const, error: usuarioError.message || "Erro ao carregar usuario do sistema." };
    if (!usuario?.id || !usuario.id_salao) return { ok: false as const, error: "Erro ao carregar usuario do sistema." };

    const securityDecision = await getSecurityAccessDecision({ tipoUsuario: "salao", userId: usuario.id, idSalao: usuario.id_salao });
    if (!securityDecision.allowed) {
      return {
        ok: false as const,
        error: getSecurityStatusMessage({ status: securityDecision.status, motivo: securityDecision.motivo, bloqueadoAte: securityDecision.bloqueadoAte }),
    ...
    const permissoesDb = sanitizePermissoesDb((permissoes as PermissoesDbRow | null) ?? null, {
    ...
    const permissoesFinal = {
      ...permissoesPadrao,
      ...permissoesDb,
      perfil_salao_ver: usuario.nivel === "admin" && (permissoesDb.perfil_salao_ver ?? permissoesPadrao.perfil_salao_ver),
      configuracoes_ver: usuario.nivel === "admin" && (permissoesDb.configuracoes_ver ?? permissoesPadrao.configuracoes_ver),
      assinatura_ver: usuario.nivel === "admin" && (permissoesDb.assinatura_ver ?? permissoesPadrao.assinatura_ver),
    };

    const resumoAssinatura = assinatura?.status ? getResumoAssinatura({ status: assinatura.status, vencimentoEm: assinatura.vencimento_em, trialFimEm: assinatura.trial_fim_em }) : null;
  ...
  ["painel-shell-context-v2"],
  { revalidate: 180, tags: ["painel-shell-context"] }
);
...
export const loadPainelShellNotificationsCached = unstable_cache(
  async (idSalao: string, resumoAssinatura: ReturnType<typeof getResumoAssinatura> | null) => {
    ...
    const notifications = buildShellNotifications({
      resumoAssinatura,
      clientes: [],
      agendamentos: ((agendamentosPendentes || []) as Array<Record<string, any>>).map((agendamento) => {
        const cliente = Array.isArray(agendamento.clientes) ? agendamento.clientes[0] : agendamento.clientes;
        const servico = Array.isArray(agendamento.servicos) ? agendamento.servicos[0] : agendamento.servicos;
        return {
          id: String(agendamento.id || ""), status: String(agendamento.status || ""), data: String(agendamento.data || ""), hora_inicio: String(agendamento.hora_inicio || ""),
          origem: String(agendamento.origem || ""), cliente_id: String(agendamento.cliente_id || ""), cliente_nome: String(cliente?.nome || "").trim() || null,
          servico_nome: String(servico?.nome || "").trim() || null, codigo_cupom: String(agendamento.codigo_cupom || "").trim() || null,
          id_cupom_salao: String(agendamento.id_cupom_salao || "").trim() || null, desconto_cupom_valor: agendamento.desconto_cupom_valor ?? null,
        };
      }),
      movimentosCaixa: [], onboarding: null, tickets: [],
    });

    for (const item of ((notificacoesOperacionais || []) as Array<Record<string, unknown>>)) {
      const tipo = String(item.tipo || "");
      notifications.push({
        id: `notification-job-${String(item.id || "")}`, title: String(item.titulo || "Notificacao do salao"), description: String(item.mensagem || "Nova notificacao operacional."),
        tone: tipo === "avaliacao_ruim" ? "warning" : "info", category: tipo.includes("avaliacao") ? "agenda" : "sistema", severity: tipo === "avaliacao_ruim" ? "high" : "medium",
        eventType: tipo || "notification_job", href: String(item.url || "/dashboard"), actionLabel: tipo === "avaliacao_ruim" ? "Ver atendimento" : "Abrir",
        destination: "internal", icon: tipo.includes("avaliacao") ? "agenda" : "alert", sourceModule: "notification_jobs", sourceEntity: "notification_jobs",
        sourceEntityId: String(item.id || ""), expiresAt: item.enviar_em ? String(item.enviar_em) : null,
      });
    }
    return notifications;
  },
  ["painel-shell-notifications-v2"],
  { revalidate: 20, tags: ["painel-shell-notifications"] }
);
...
export async function loadPainelShellData() {
  ...
  if (!result.ok) {
    if ("redirectPath" in result && result.redirectPath) redirect(result.redirectPath);
    return result;
  }
  if (result.data.status !== "ativo") redirect(buildLoginRedirectUrl("usuario_inativo"));

  const notifications = await loadPainelShellNotificationsCached(result.data.idSalao, result.data.resumoAssinatura);
  return {
    ok: true as const,
    data: {
      ...result.data,
