...
begin
  ...
  loop
    ...
    exception
      when others then
        raise exception 'Falha ao atualizar item % da comanda %: %', v_item.id, p_id_comanda, SQLERRM
    ...
    then
      ...
      exception
        when others then
          raise exception 'Falha ao inserir comissao do profissional para item % da comanda %: %', v_item.id, p_id_comanda, SQLERRM
            using errcode = SQLSTATE;
      end;
    end if;
    ...
    if v_assistente_vinculado and v_percentual_assistente > 0 then
      ...
      exception
        when others then
          raise exception 'Falha ao inserir lancamento do assistente para item % da comanda %: %', v_item.id, p_id_comanda, SQLERRM
            using errcode = SQLSTATE;
      end;
      ...
      exception
        when others then
          raise exception 'Falha ao inserir comissao assistente para item % da comanda %: %', v_item.id, p_id_comanda, SQLERRM
            using errcode = SQLSTATE;
      end;
    end if;
  end loop;
