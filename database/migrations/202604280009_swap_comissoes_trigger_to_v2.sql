...
as $$
begin
  perform public.fn_gerar_comissoes_comanda_v2(p_id_comanda);
end;
$$;

create or replace function public.fn_sync_comissoes_status_comanda_v2()
returns trigger
language plpgsql
security definer
set search_path = public
...
begin
  ...
  then
    begin
      perform public.fn_gerar_comissoes_comanda_v2(new.id);
    exception
      when others then
...
for each row
execute function public.fn_sync_comissoes_status_comanda_v2();
