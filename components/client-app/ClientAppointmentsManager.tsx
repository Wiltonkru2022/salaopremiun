...
import { Bell, CalendarDays, Clock3, MessageCircle, X, XCircle } from "lucide-react";
...
function formatDuration(item: ClientAppAppointmentListItem) {
  ...
  return formatClientDuration(
    Math.max(0, end[0] * 60 + end[1] - (start[0] * 60 + start[1]))
  );
}
...
function canConfirmPresence(item: ClientAppAppointmentListItem) {
  return (
    ...
    String(item.confirmacaoClienteStatus || "").toLowerCase() !== "confirmado"
  );
}
...
function buildWhatsappLink(item: ClientAppAppointmentListItem) {
  ...
  const message = encodeURIComponent(
    `Olá, ${item.salaoNome}. Quero falar sobre meu agendamento de ${item.servicoNome} no dia ${item.data
      .split("-")
      .reverse()
      .join("/")} às ${item.horaInicio.slice(0, 5)}.`
  );
...
function ActionButton({
  ...
  return (
    <button
      ...
      className={`inline-flex min-h-12 items-center justify-center gap-2 rounded-2xl px-4 text-sm font-black transition disabled:opacity-60 ${toneClass}`}
    >
...
function CancelAppointmentForm({ idAgendamento }: { idAgendamento: string }) {
  ...
  const [state, formAction] = useActionState<ClienteAppointmentActionState, FormData>(
    cancelClienteAppointmentAction,
    { error: null }
  );
  ...
  if (!confirming) {
    return (
      <button
        ...
        className="inline-flex min-h-12 items-center justify-center gap-2 rounded-2xl border border-red-200 bg-red-50 px-4 text-sm font-black text-red-700"
      >
        <XCircle size={19} /> Cancelar
      </button>
    );
  }

  return (
    <div className="fixed inset-0 z-[1000] flex items-end justify-center bg-black/55 p-3 backdrop-blur-[2px] sm:items-center">
      <div
        role="dialog"
        aria-modal="true"
        aria-label="Cancelar agendamento"
        className="w-full max-w-md rounded-[1.5rem] bg-white p-5 shadow-2xl"
      >
        <div className="flex items-start justify-between gap-3">
          <div>
            <div className="text-xs font-black uppercase tracking-[0.16em] text-red-600">Atenção</div>
            <h3 className="mt-1 text-2xl font-black text-zinc-950">Cancelar agendamento?</h3>
          </div>
          <button
            type="button"
            onClick={() => setConfirming(false)}
            className="grid h-10 w-10 place-items-center rounded-full bg-zinc-100"
            aria-label="Fechar"
          >
            <X size={20} />
          </button>
        ...
        <p className="mt-3 text-sm leading-6 text-zinc-600">
          O horário será liberado. Se quiser apenas outro horário, prefira reagendar.
        </p>
        <form action={formAction} className="mt-5 grid grid-cols-2 gap-2">
          ...
          <button
            type="button"
            onClick={() => setConfirming(false)}
            className="min-h-12 rounded-2xl border border-zinc-200 bg-white text-sm font-black"
          >
            Voltar
          </button>
          <ActionButton label="Sim, cancelar" pendingLabel="Cancelando..." tone="danger" />
        </form>
        {state.error ? (
  ...
  return (
    <div className="space-y-3">
      <button
        ...
        className="inline-flex min-h-12 items-center justify-center gap-2 rounded-2xl border border-zinc-200 bg-white px-4 text-sm font-black text-zinc-950"
      >
        <CalendarDays size={19} /> Reagendar
      </button>
      {open ? (
        <form action={formAction} className="space-y-4 rounded-2xl border border-zinc-200 bg-zinc-50 p-4">
          ...
          {loading ? (
            <div className="text-sm text-zinc-500">Buscando horários livres...</div>
          ) : error ? (
                ...
                <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                  {dias.map((dia) => (
                    <button
                      ...
                      className={`min-h-12 rounded-xl border px-3 py-2 text-left text-sm font-black ${
                ...
                <div className="grid grid-cols-3 gap-2 sm:grid-cols-4">
                  {horarios.map((horario) => (
                    <button
                      ...
                      className={`min-h-11 rounded-xl border px-2 py-2 text-sm font-black ${
            ...
            <div className="text-sm text-zinc-500">Sem horários livres para esse atendimento.</div>
          )}
          {state.error ? (
            <div className="rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-xs text-red-700">
              {state.error}
        ...
        {agendamentos.length ? (
          agendamentos.map((item) => {
            ...
            return (
              <article
                key={item.id}
                className="overflow-hidden rounded-[1.5rem] border border-zinc-200 bg-white shadow-[0_12px_34px_rgba(15,23,42,0.07)]"
              >
                <div className="p-4 sm:p-5">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <div className="flex flex-wrap gap-2">
                        <span className="inline-flex items-center gap-2 rounded-full bg-zinc-100 px-3 py-1.5 text-xs font-black text-zinc-700">
                          <Bell size={14} /> {formatStatus(item.status)}
                        </span>
                      ...
                      <h2 className="mt-3 break-words text-xl font-black tracking-[-0.035em] text-zinc-950">
                        {item.servicoNome}
                      </h2>
                      <p className="mt-1 text-sm font-semibold text-zinc-500">
                        {item.salaoNome} · {item.profissionalNome}
                      </p>
                    </div>
                    <div className="shrink-0 rounded-2xl bg-zinc-950 px-3 py-2 text-center text-white">
                      <div className="text-[10px] font-black tracking-wider text-amber-300">{formatMonth(item.data)}</div>
                      <div className="text-2xl font-black">{formatDay(item.data)}</div>
                      <div className="text-xs font-bold">{item.horaInicio.slice(0, 5)}</div>
                    </div>
                  </div>
                  <div className="mt-4 grid gap-2 text-sm text-zinc-600 sm:grid-cols-2">
                    <div className="flex items-center gap-2">
                      <Clock3 size={17} /> <span>{formatDuration(item)}</span>
                    </div>
                    {formatCreatedAt(item.criadoEm) ? (
                      <div className="flex items-center gap-2 text-xs text-zinc-400 sm:col-span-2">
                        <CalendarDays size={15} /> Solicitado em {formatCreatedAt(item.criadoEm)}
                      </div>
                    ) : null}
                  </div>
                  {item.podeCancelar ? (
                    <div className="mt-4 rounded-xl bg-zinc-50 px-3 py-2 text-xs font-bold text-zinc-600">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {needsSignalPayment(item) ? (
                      <Link
                        ...
                        className="inline-flex min-h-12 items-center rounded-2xl bg-emerald-600 px-4 text-sm font-black text-white"
                      >
                        Pagar sinal
                      </Link>
                    ) : null}
                    ...
                    {canBookAgain(item) && !item.podeCancelar ? (
                      <Link
                        ...
                        className="inline-flex min-h-12 items-center rounded-2xl bg-zinc-950 px-4 text-sm font-black text-white"
                      >
                        Reservar novamente
                      </Link>
                    ) : null}
                    {whatsapp ? (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="inline-flex min-h-12 items-center gap-2 rounded-2xl border border-emerald-200 bg-emerald-50 px-4 text-sm font-black text-emerald-700"
                      >
          ...
          <div className="rounded-[1.5rem] border border-dashed border-zinc-300 bg-zinc-50 p-7 text-center">
            <CalendarDays size={42} className="mx-auto text-zinc-400" />
            <h2 className="mt-4 text-xl font-black text-zinc-950">Nenhum agendamento ainda</h2>
            <p className="mt-2 text-sm leading-6 text-zinc-500">Explore os salões e reserve seu próximo horário.</p>
            <Link
              href="/app-cliente/explorar"
              className="mt-5 inline-flex min-h-12 items-center rounded-2xl bg-zinc-950 px-5 text-sm font-black text-white"
            >
              Encontrar um salão
            </Link>
          </div>
