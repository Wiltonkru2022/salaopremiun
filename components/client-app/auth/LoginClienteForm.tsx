...
export default function LoginClienteForm({
  ...
  return (
    <div className="space-y-3">
      <form action={formAction} className="overflow-hidden rounded-[1.9rem] border border-white/70 bg-white p-5 shadow-[0_18px_48px_rgba(15,23,42,0.08)]">
        ...
        <div className="mt-5 space-y-4">
          <div>
            ...
            <input name="cpf" type="text" inputMode="numeric" autoComplete="off" placeholder="000.000.000-00" required maxLength={14} className="h-12 w-full rounded-2xl border border-zinc-200 bg-white px-4 text-base outline-none transition focus:border-zinc-400" />
          </div>
          <div>
            <label className="mb-1.5 block text-sm font-medium text-zinc-700">Data de nascimento</label>
            <input name="dataNascimento" type="text" inputMode="numeric" autoComplete="bday" placeholder="DD/MM/AAAA" required maxLength={10} className="h-12 w-full rounded-2xl border border-zinc-200 bg-white px-4 text-base outline-none transition focus:border-zinc-400" />
          </div>

          {oauthError ? <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">{oauthError}</div> : null}
          ...
          <div className="space-y-2 pt-1 text-center">
            <Link href="/app-cliente/recuperar-acesso" className="block text-sm font-medium text-zinc-700 underline underline-offset-4">Problemas para acessar?</Link>
            <Link
              href={salaoId ? `/app-cliente/cadastro?salao=${salaoId}${next ? `&next=${encodeURIComponent(next)}` : ""}` : next ? `/app-cliente/cadastro?next=${encodeURIComponent(next)}` : "/app-cliente/cadastro"}
      ...
      <details className="rounded-[1.5rem] border border-zinc-200 bg-zinc-50 p-4 text-zinc-700">
        ...
        <p className="mt-2 text-xs leading-5 text-zinc-500">Use esta opção somente para entrar uma última vez e completar CPF e data de nascimento.</p>
        <form action={legacyFormAction} className="mt-4 space-y-3">
          {salaoId ? <input type="hidden" name="salao" value={salaoId} /> : null}
          {next ? <input type="hidden" name="next" value={next} /> : null}
          <input name="identidade" type="text" placeholder="Telefone ou e-mail antigo" required className="h-11 w-full rounded-xl border border-zinc-200 bg-white px-3 text-sm outline-none" />
          <input name="senha" type="password" placeholder="Senha antiga" required className="h-11 w-full rounded-xl border border-zinc-200 bg-white px-3 text-sm outline-none" />
          {legacyState.error ? <div className="rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-xs text-red-700">{legacyState.error}</div> : null}
          <SubmitButton legacy />
        </form>
      </details>
    </div>
