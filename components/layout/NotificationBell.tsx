...
function NotificationIcon({
  ...
  const className = clsx("h-9 w-9 rounded-2xl p-2 ring-1", toneClass[tone]);
...
function getSharedCookieDomain() {
  ...
  if (hostname === "salaopremiun.com.br" || hostname.endsWith(".salaopremiun.com.br")) {
...
export default function NotificationBell({
  ...
  useEffect(() => {
    ...
    try {
      ...
      const next = Array.isArray(parsed)
        ...
        : [];
      setReadIds(next);
      if (!cookieReadIds && next.length) persistReadIdsCookie(cookieName, next);
    } catch {
      setReadIds([]);
    } finally {
      setHydrated(true);
  ...
  const markAllAsRead = () => {
    persistReadIds(
      allNotifications
        .filter((notification) => !notification.critical && !notification.persistUntilResolved)
  ...
  const visibleNotifications = useMemo(
    () =>
      [...allNotifications]
        ...
        .sort(
          (a, b) =>
            Number(Boolean(b.critical || b.persistUntilResolved)) -
            Number(Boolean(a.critical || a.persistUntilResolved))
        )
        .slice(0, 8),
  ...
  useEffect(() => {
    ...
    if (validIds.length !== readIds.length) persistReadIds(validIds);
  }, [allNotifications, hydrated, persistReadIds, readIds]);

  return (
    <div className={clsx("relative", open && "z-[100]")} ref={wrapperRef}>
      <button
        type="button"
...
function NotificationItem({
  ...
  const content = (
    <>
      ...
      {notification.href ? <ChevronRight size={16} className="text-zinc-300" /> : null}
    </>
  );
  ...
  if (!notification.href) {
    return (
      <button type="button" onClick={onOpen} className={clsx("w-full text-left", className)}>
        {content}
      </button>
    );
  }

  const href = getNotificationHref(notification.href);
  const target = getWorkspaceWindowTarget(notification.href);
  const standalone = isPainelStandaloneWindow();

  if (isExternalHref(href) || (target && !standalone)) {
    return (
      <a
        href={href}
        target={target && !standalone ? target : undefined}
        onClick={(event) => {
          onOpen();
          if (target) {
            event.preventDefault();
            openPainelWorkspaceWindow(notification.href || href);
          }
        }}
        className={className}
      >
  ...
  return (
    <Link href={href} prefetch onClick={onOpen} className={className}>
      {content}
    </Link>
  );
}
