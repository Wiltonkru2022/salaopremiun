...
const COMMANDS = [
  ...
  { label: "Criar campanha", description: "Abrir gestão de campanhas", href: "/admin-master/campanhas", icon: Sparkles },
    ...
    <>
      ...
      {open ? (
        <div className="fixed inset-0 z-[90] flex items-start justify-center bg-zinc-950/35 p-3 pt-[8vh] backdrop-blur-sm sm:p-6 sm:pt-[12vh]" onMouseDown={() => setOpen(false)}>
          <div className="w-full max-w-2xl overflow-hidden rounded--xl border border-zinc-200 bg-white shadow-2xl" onMouseDown={(event) => event.stopPropagation()}>
            <div className="flex items-center gap-3 border-b border-zinc-100 px-4 py-3 sm:px-5">
              ...
              <input
                ...
                placeholder="Salão, ticket, cobrança... ou digite uma ação"
              ...
              {commands.length ? (
                <div>
                  ...
                  <div className="grid gap-1 sm:grid-cols-2">
                    {commands.slice(0, deferredQuery ? 4 : 7).map((command) => {
              ...
              {deferredQuery.length >= 2 ? (
                <div className="mt-3 border-t border-zinc-100 pt-3">
                  ...
                  {!loading && results.length ? results.map((result) => (
                    <Link key={`${result.type}-${result.id}`} href={result.href} onClick={() => setOpen(false)} className="flex items-start gap-3 rounded-2xl px-3 py-3 transition hover:bg-zinc-50">
                      ...
                      <span className="min-w-0"><span className="block truncate text-sm font-black text-zinc-950">{result.title}</span><span className="mt-1 block text-xs leading-5 text-zinc-500">{result.subtitle}</span></span>
                    </Link>
                  )) : null}
                  {!loading && !results.length ? <div className="rounded-2xl bg-zinc-50 px-4 py-5 text-sm text-zinc-500">Nenhum resultado encontrado. Tente nome, número do ticket, e-mail ou salão.</div> : null}
                </div>
              ) : (
