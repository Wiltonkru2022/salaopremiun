...
export default function PartnerOffersGrid({
  publico,
  idSalao,
}: {
  ...
  const filtered = useMemo(() => {
    ...
    if (!term) return rows;
    return rows.filter((row) => `${row.parceiro} ${row.titulo} ${row.subtitulo || ""} ${row.cupomCodigo || ""}`.toLowerCase().includes(term));
  }, [query, rows]);
  ...
  function trackAndOpen(row: Campanha) {
    void fetch("/api/parcerias/ativos", {
      ...
      body: JSON.stringify({ idCampanha: row.id, local: "parceiros", tipo: "clique" }),
      keepalive: true,
    });
  ...
  return (
    <div className="space-y-5">
      <div className="relative">
        ...
        <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar parceiro, oferta ou cupom" className="h-12 w-full rounded-2xl border border-zinc-200 bg-white pl-11 pr-4 text-sm outline-none transition focus:border-zinc-400" />
      </div>
      ...
      {!loading && !filtered.length ? <div className="rounded-[24px] border border-dashed border-zinc-300 bg-white p-8 text-center"><h2 className="text-lg font-black text-zinc-950">Nenhum benefício disponível agora</h2><p className="mt-2 text-sm text-zinc-500">Quando houver parceiros compatíveis com seu perfil ou região, eles aparecerão aqui.</p></div> : null}

      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {filtered.map((row) => (
          <article key={row.id} className="overflow-hidden rounded-[24px] border border-zinc-200 bg-white shadow-sm">
            {row.imagemUrl ? <img src={row.imagemUrl} alt={row.altText || row.titulo} className="h-44 w-full object-cover" loading="lazy" /> : <div className="flex h-36 items-center justify-center bg-zinc-950 px-6 text-center text-sm font-black uppercase tracking-[0.16em] text-amber-200">Parceiro Salão Premium</div>}
            <div className="p-5">
              <div className="text-[10px] font-black uppercase tracking-[0.2em] text-amber-700">Parceiro Salão Premium</div>
