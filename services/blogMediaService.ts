import { removeCloudinaryAssetByUrl, uploadBufferToCloudinary } from "@/lib/platform/cloudinary.server";
...
export async function removeBlogMedia(publicUrl: string) {
  if (!publicUrl.includes("res.cloudinary.com")) {
    throw new Error("Somente midias Cloudinary podem ser removidas neste ambiente.");
  }

  const removed = await removeCloudinaryAssetByUrl(publicUrl);
  if (!removed) {
    throw new Error("URL Cloudinary invalida para remocao.");
  }
}
