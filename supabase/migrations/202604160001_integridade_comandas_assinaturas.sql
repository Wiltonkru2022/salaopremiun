...
create unique index if not exists assinaturas_cobrancas_auto_ciclo_unico_idx
on public.assinaturas_cobrancas (id_assinatura, data_expiracao, tipo_movimento)
where coalesce(gerada_automaticamente, false) = true
  and coalesce(deleted, false) = false
  and lower(coalesce(status, '')) in ('pending', 'pendente', 'aguardando_pagamento');
