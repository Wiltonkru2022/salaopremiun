alter table public.comanda_pagamentos
  ...
  add column if not exists destino_excedente text;

alter table public.comanda_pagamentos
  drop constraint if exists comanda_pagamentos_destino_excedente_check;

alter table public.comanda_pagamentos
  add constraint comanda_pagamentos_destino_excedente_check
  check (
    destino_excedente is null
    or destino_excedente in ('troco', 'credito_cliente')
  );

create or replace function public.fn_gerar_comissoes_comanda_v2(p_id_comanda uuid)
returns void
language plpgsql
security definer
...
declare
  ...
  v_comanda_desconto numeric(12, 2);
  v_comanda_acrescimo numeric(12, 2);
  v_competencia date;
  ...
  v_ajuste_desconto_item numeric(12, 2);
  v_ajuste_acrescimo_item numeric(12, 2);
  v_valor_item_ajustado numeric(12, 2);
...
begin
  select
    ...
    c.fechada_em,
    c.id_salao,
    round(coalesce(c.desconto, 0)::numeric, 2),
    round(coalesce(c.acrescimo, 0)::numeric, 2)
  into
    v_comanda_status,
  ...
  loop
    ...
    if v_subtotal_itens > 0 then
      v_ajuste_desconto_item := round((v_comanda_desconto * coalesce(v_item.valor_total, 0)::numeric) / v_subtotal_itens, 2);
      v_ajuste_acrescimo_item := round((v_comanda_acrescimo * coalesce(v_item.valor_total, 0)::numeric) / v_subtotal_itens, 2);
    else
      v_ajuste_desconto_item := 0;
      v_ajuste_acrescimo_item := 0;
    end if;

    v_valor_item_ajustado := greatest(
      round(coalesce(v_item.valor_total, 0)::numeric - v_ajuste_desconto_item + v_ajuste_acrescimo_item, 2),
      0
    );
    ...
    if lower(coalesce(
      ...
      v_base_original := greatest(
        round(v_valor_item_ajustado - coalesce(v_item.custo_total, 0)::numeric, 2),
        0
      );
    else
      v_base_original := v_valor_item_ajustado;
    end if;
    ...
    set
      ...
      base_calculo_aplicada = coalesce(v_vinculo_base_calculo, v_servico_base_calculo, v_item.base_calculo_aplicada, 'bruto'),
      desconta_taxa_maquininha_aplicada = coalesce(v_vinculo_desconta_taxa, v_servico_desconta_taxa, v_item.desconta_taxa_maquininha_aplicada, false),
      updated_at = now()
    where id = v_item.id;

    if v_item.id_profissional is not null
      and (lower(coalesce(v_item.tipo_item, '')) = 'servico' or v_percentual > 0)
    then
      insert into public.comissoes_lancamentos (
        id, id_salao, id_comanda, id_comanda_item, id_profissional, id_assistente,
        tipo_destinatario, tipo_profissional, descricao, percentual,
        percentual_aplicado, valor_base, valor_comissao, valor_comissao_assistente,
        status, competencia_data, observacoes, origem_percentual, criado_em, updated_at
      )
      values (
        gen_random_uuid(), v_comanda_id_salao, p_id_comanda, v_item.id,
        v_item.id_profissional,
        case when v_assistente_vinculado then v_item.id_assistente else null end,
        'profissional', 'profissional', coalesce(v_item.descricao, 'Servico'),
        v_percentual, v_percentual, v_base_calculo, v_valor_profissional_final, 0,
        'pendente', v_competencia,
        concat(
          'Calculada no fechamento. Base ajustada: ', v_base_calculo,
          '; desconto rateado: ', v_ajuste_desconto_item,
          '; acrescimo rateado: ', v_ajuste_acrescimo_item,
          '; taxa descontada: ', round(v_taxa_item, 2),
          '; assistente descontado: ', round(v_valor_assistente, 2), '.'
        ),
        v_origem, coalesce(v_comanda_fechada_em, now()), now()
      );
    end if;
    ...
    if v_assistente_vinculado and v_percentual_assistente > 0 then
      ...
      values (
        v_id_lancamento_assistente, v_comanda_id_salao, p_id_comanda, v_item.id,
        v_item.id_assistente, v_item.id_assistente,
        'assistente', 'assistente', coalesce(v_item.descricao, 'Servico') || ' - assistente',
        v_percentual_assistente, v_percentual_assistente, v_base_calculo,
        v_valor_assistente, v_valor_assistente, 'pendente', v_competencia,
        concat('Calculada no fechamento. Base ajustada por desconto/acrescimo: ', v_base_calculo, '.'),
        'assistente', coalesce(v_comanda_fechada_em, now()), now()
      );

      insert into public.comissoes_assistentes (
        id, id_salao, id_comanda, id_comanda_item, id_profissional, id_assistente,
        percentual, valor_base, valor_comissao, status, competencia_data,
        observacoes, created_at, updated_at
      )
      values (
        gen_random_uuid(), v_comanda_id_salao, p_id_comanda, v_item.id,
        v_item.id_profissional, v_item.id_assistente, v_percentual_assistente,
        v_base_calculo, v_valor_assistente, 'pendente', v_competencia,
        'Gerada automaticamente no fechamento da comanda com base ajustada por desconto/acrescimo.',
        coalesce(v_comanda_fechada_em, now()), now()
      );
    end if;
  end loop;
end;
...
revoke all on function public.fn_gerar_comissoes_comanda_v2(uuid) from public, anon, authenticated;
grant execute on function public.fn_gerar_comissoes_comanda_v2(uuid) to service_role;
