...
export default function ComissoesPage() {
  ...
  const comissaoAberta = useMemo(
    () => rows.find((item) => item.id === comissaoAbertaId) || null,
    [comissaoAbertaId, rows]
  );
  const comissaoAbertaNome =
    comissaoAberta?.profissionais?.nome || "Profissional";
  const comissaoAbertaOrigem = comissaoAberta
    ? origemMeta(comissaoAberta.origem_percentual)
    : null;
  const comissaoAbertaStatus = comissaoAberta
    ? getStatusComissaoMeta(comissaoAberta.status)
    : null;
  const comissaoAbertaDetalhe = comissaoAberta
    ? getPrintDetail(comissaoAberta)
    : null;
  ...
  useEffect(() => {
    if (comissaoAbertaId && !comissaoAberta) {
          ...
          >
            <div className="border-b border-zinc-200 px-5 py-4">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <h2
                    id="comissao-detalhe-title"
                    className="mt-1 text-2xl font-black tracking-[-0.04em] text-zinc-950"
                  >
                    {formatCurrency(getValorLancamento(comissaoAberta))}
                  </h2>
                  <p className="mt-1 text-sm text-zinc-500">
                    Conferência do cálculo antes de pagar ou cancelar.
                  </p>
                </div>
                <button
                  type="button"
            ...
            <div className="space-y-4 px-5 py-5">
              <div className="grid gap-2.5 sm:grid-cols-2">
                <DetailBox label="Profissional" value={comissaoAbertaNome} />
                <DetailBox
                  label="Tipo"
                  value={
                    getTipoDestinatario(comissaoAberta) === "assistente"
                      ? "Assistente"
                      : "Profissional"
                  }
                />
                <DetailBox
                  label="Descrição"
                  value={parseComboDisplayMeta(comissaoAberta.descricao).displayTitle}
                />
                <DetailBox
                  label="Competência"
                  value={formatDate(comissaoAberta.competencia_data)}
                />
              </div>
              ...
              <div className="rounded-[22px] border border-zinc-200 bg-zinc-50 p-4">
                ...
                <div className="mt-3 grid gap-2.5 sm:grid-cols-3">
                  <DetailBox
                    label="Base"
                    value={formatCurrency(comissaoAberta.valor_base)}
                    compact
                  />
                  <DetailBox
                    label="Percentual"
                    value={formatPercent(comissaoAberta.percentual_aplicado)}
                    compact
                  />
                  <DetailBox
                    label="Comissão"
                    value={formatCurrency(getValorLancamento(comissaoAberta))}
                    compact
                    strong
                  />
                </div>

                {comissaoAbertaDetalhe ? (
                  <div className="mt-3 grid gap-2.5 sm:grid-cols-2">
                    <DetailBox
                      label="Desconto rateado"
                      value={formatCurrency(comissaoAbertaDetalhe.descontoRateado)}
                      compact
                    />
                    <DetailBox
                      label="Acréscimo rateado"
                      value={formatCurrency(comissaoAbertaDetalhe.acrescimoRateado)}
                      compact
                    />
                    <DetailBox
                      label="Taxa descontada"
                      value={formatCurrency(comissaoAbertaDetalhe.taxa)}
                      compact
                    />
                    <DetailBox
                      label="Assistente descontado"
                      value={formatCurrency(comissaoAbertaDetalhe.assistente)}
                      compact
                    />
                  </div>
                ) : null}
              ...
              <div className="grid gap-2.5 sm:grid-cols-2">
                <div className="rounded-[20px] border border-zinc-200 bg-white p-3">
                  ...
                  {comissaoAbertaOrigem ? (
                    <>
                      <span
                        className={`mt-2 inline-flex rounded-full border px-3 py-1 text-xs font-semibold ${comissaoAbertaOrigem.badgeClass}`}
                      >
                        {comissaoAbertaOrigem.label}
                      </span>
                      <p className="mt-2 text-sm leading-5 text-zinc-500">
                        {comissaoAbertaOrigem.description}
                      </p>
                    </>
                  ) : null}
                </div>
                ...
                <div className="rounded-[20px] border border-zinc-200 bg-white p-3">
                  ...
                  {comissaoAbertaStatus ? (
                    <>
                      <span
                        className={`mt-2 inline-flex rounded-full border px-3 py-1 text-xs font-semibold ${comissaoAbertaStatus.badgeClass}`}
                      >
                        {comissaoAbertaStatus.label}
                      </span>
                      <p className="mt-2 text-sm leading-5 text-zinc-500">
                        {comissaoAbertaStatus.description}
                      </p>
                    </>
                  ) : null}
                </div>
              ...
              <div className="rounded-[20px] border border-zinc-200 bg-zinc-50 p-3">
                ...
                <p className="mt-2 text-sm leading-6 text-zinc-600">
                  {comissaoAberta.observacoes || "Sem observações adicionais."}
                </p>
              </div>
            </div>

            <div className="sticky bottom-0 flex flex-col gap-2 border-t border-zinc-200 bg-white/95 px-5 py-4 backdrop-blur sm:flex-row sm:justify-end">
              {podeGerenciar &&
              normalizeStatusComissao(comissaoAberta.status) !== "cancelado" ? (
                <button
                  type="button"
                  disabled={saving}
                  onClick={() => {
              ...
              {podeGerenciar &&
              normalizeStatusComissao(comissaoAberta.status) === "pendente" ? (
                <button
                  type="button"
                  disabled={saving}
                  onClick={() => {
              ...
              <table className="w-full min-w-[980px] table-fixed text-[13px]">
                ...
                <tbody>
                  {rows.length === 0 ? (
                    <tr>
                      <td
                      colSpan={comissoesAvancadas ? 8 : 7}
...
function DetailBox({
  ...
  return (
    <div className="rounded-[18px] border border-zinc-200 bg-white px-3 py-2.5">
