...
type Resumo = { total: number; pendente: number; pago: number; cancelado: number };
type ComissaoRow = {
  id: string;
...
function getTipoDestinatario(item: ComissaoRow) {
  return String(item.tipo_destinatario || item.tipo || "").toLowerCase() === "assistente"
    ? "assistente"
    : "profissional";
}
...
export default function ComissoesPage() {
  ...
  const [dataInicial, setDataInicial] = useState(
    `${primeiroDiaMes.getFullYear()}-${String(primeiroDiaMes.getMonth() + 1).padStart(2, "0")}-${String(primeiroDiaMes.getDate()).padStart(2, "0")}`
  );
  ...
  const [resumo, setResumo] = useState<Resumo>({ total: 0, pendente: 0, pago: 0, cancelado: 0 });
  ...
  const carregarComissoes = useCallback(async (salaoIdParam?: string) => {
    try {
      ...
      const idsProfissionais = Array.from(new Set(baseRows.map((item) => item.id_profissional).filter(Boolean))) as string[];
      ...
      if (idsProfissionais.length > 0) {
        ...
        mapaProfissionais = new Map((((profissionaisData as { id: string; nome: string }[]) || [])).map((item) => [item.id, item]));
      }
      ...
      const enriched = baseRows
        ...
        .filter((item) => {
          ...
          return profissional.includes(termo) || descricao.includes(termo) || origemMeta(item.origem_percentual).label.toLowerCase().includes(termo);
        });
      ...
      console.error(error);
      setErro(error instanceof Error ? error.message : "Erro ao carregar comissoes.");
    }
  }, [busca, dataFinal, dataInicial, idSalao, profissionalId, status, supabase, tipoDestinatario]);
  ...
  const init = useCallback(async () => {
    try {
      ...
      const { data: profissionaisData, error: profissionaisError } = await supabase.from("profissionais").select("id, nome").eq("id_salao", salaoIdFinal).order("nome");
  ...
  async function cancelarLancamento(id: string) {
    if (!podeGerenciar) return setErro("Voce nao tem permissao para cancelar lancamento.");
  ...
  function imprimirRateio() {
    ...
    if (!win) return;
    win.document.write(`<html><body><h1>Rateio de Comissoes</h1><p>Periodo: ${formatDate(dataInicial)} ate ${formatDate(dataFinal)}</p><p>Total: ${formatCurrency(resumo.total)}</p></body></html>`);
    win.document.close();
    win.focus();
    win.print();
  }
  ...
  const resumoProfissionais = useMemo(() => {
    const mapa = new Map<string, { id: string; nome: string; total: number; pendente: number; quantidade: number; statusPredominante: string; statusMap: Record<string, number> }>();
    rows.forEach((item) => {
      const id = item.id_profissional || item.profissionais?.nome || item.id;
      const nome = item.profissionais?.nome || "Profissional";
      const valor = getValorLancamento(item);
      if (!mapa.has(id)) mapa.set(id, { id, nome, quantidade: 0, pendente: 0, statusMap: {}, statusPredominante: "pendente", total: 0 });
  ...
  if (loading || !acessoCarregado) return <div className="p-6">Carregando comissoes...</div>;
      ...
      <div className="mx-auto max-w-[1800px] space-y-5">
        <div className="overflow-hidden rounded-[28px] border border-zinc-200 bg-white shadow-sm">
          ...
          <div className="grid gap-3 px-4 py-4 md:grid-cols-2 xl:grid-cols-7">
            <ResumoCard title="Total" value={formatCurrency(resumo.total)} icon={<BadgeDollarSign size={16} />} tone="zinc" />
            <ResumoCard title="Profissionais" value={formatCurrency(resumoPorTipo.profissional)} icon={<User2 size={16} />} tone="sky" />
            <ResumoCard title="Assistentes" value={formatCurrency(resumoPorTipo.assistente)} icon={<Layers3 size={16} />} tone="emerald" />
            <ResumoCard title="Pendente" value={formatCurrency(resumo.pendente)} icon={<CalendarDays size={16} />} tone="amber" />
            <ResumoCard title="Pago" value={formatCurrency(resumo.pago)} icon={<CheckCircle2 size={16} />} tone="emerald" />
            <ResumoCard title="Ticket medio" value={formatCurrency(ticketMedio)} icon={<WalletCards size={16} />} tone="sky" />
            <ResumoCard title="Maior lancamento" value={formatCurrency(maiorLancamento ? getValorLancamento(maiorLancamento) : 0)} icon={<Sparkles size={16} />} tone="violet" />
          </div>
        </div>

        <ComissaoHelpPanel eyebrow="Leitura rapida" title="Como este valor foi definido" description="A origem mostra qual regra entrou no calculo e evita adivinhacao na hora de conferir o rateio." steps={[{ title: "Padrao do servico", description: "Esse deve ser o caminho mais comum para a comissao." }, { title: "Excecao do profissional", description: "Quando um vinculo foge do padrao, a tela aponta isso." }, { title: "Taxa da maquininha", description: "So entra na conta se a configuracao permitir." }]} />
        ...
        <div className="rounded-[28px] border border-zinc-200 bg-white p-5 shadow-sm">
          <div className="grid grid-cols-1 gap-4 xl:grid-cols-[1.1fr_190px_190px_240px_180px_180px_auto]">
            <Field label="Buscar"><div className="relative"><Search size={16} className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-zinc-400" /><input value={busca} onChange={(e) => setBusca(e.target.value)} placeholder="Profissional, descricao ou origem" className="w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-11 py-3 text-sm outline-none transition focus:border-zinc-900 focus:bg-white" /></div></Field>
            <Field label="Status"><select value={status} onChange={(e) => setStatus(e.target.value)} className="w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm outline-none transition focus:border-zinc-900 focus:bg-white"><option value="todos">Todos</option><option value="pendente">Pendente</option><option value="pago">Pago</option><option value="cancelado">Cancelado</option></select></Field>
            <Field label="Tipo"><select value={tipoDestinatario} onChange={(e) => setTipoDestinatario(e.target.value)} className="w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm outline-none transition focus:border-zinc-900 focus:bg-white"><option value="todos">Todos</option><option value="profissional">Profissional</option><option value="assistente">Assistente</option></select></Field>
            <Field label="Profissional"><select value={profissionalId} onChange={(e) => setProfissionalId(e.target.value)} className="w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm outline-none transition focus:border-zinc-900 focus:bg-white"><option value="">Todos</option>{profissionais.map((item) => <option key={item.id} value={item.id}>{item.nome}</option>)}</select></Field>
            <Field label="Data inicial"><input type="date" value={dataInicial} onChange={(e) => setDataInicial(e.target.value)} className="w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm outline-none transition focus:border-zinc-900 focus:bg-white" /></Field>
            <Field label="Data final"><input type="date" value={dataFinal} onChange={(e) => setDataFinal(e.target.value)} className="w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm outline-none transition focus:border-zinc-900 focus:bg-white" /></Field>
            <Field label="Aplicar"><button onClick={() => void carregarComissoes()} className="w-full rounded-2xl bg-zinc-900 px-4 py-3 text-sm font-semibold text-white transition hover:opacity-95">Atualizar</button></Field>
          </div>
          <div className="mt-4 flex flex-wrap gap-2">
            <button onClick={apurarRateio} className="rounded-2xl border border-zinc-200 bg-white px-4 py-3 text-sm font-semibold text-zinc-800 transition hover:bg-zinc-50">Apurar rateio</button>
            <button onClick={imprimirRateio} className="inline-flex items-center gap-2 rounded-2xl border border-zinc-200 bg-white px-4 py-3 text-sm font-semibold text-zinc-800 transition hover:bg-zinc-50"><Printer size={16} />Imprimir rateio</button>
            {podeGerenciar ? <button onClick={marcarFiltradasComoPagas} disabled={saving || totalPendentesCount === 0} className="rounded-2xl bg-zinc-900 px-4 py-3 text-sm font-semibold text-white transition hover:opacity-95 disabled:opacity-50">Marcar filtradas como pagas</button> : null}
          </div>
        </div>
        ...
        <div className="overflow-hidden rounded-[28px] border border-zinc-200 bg-white shadow-sm">
          <div className="border-b border-zinc-200 px-5 py-4"><div className="text-xs font-semibold uppercase tracking-[0.18em] text-zinc-500">Leitura analitica</div><div className="mt-1 text-xl font-bold text-zinc-950">Lancamentos detalhados</div></div>
          <div className="overflow-x-auto">
            <table className="min-w-[1450px] w-full">
              <thead><tr className="border-b border-zinc-200 bg-zinc-50 text-left text-xs uppercase tracking-[0.14em] text-zinc-500"><th className="px-5 py-4">Pessoa</th><th className="px-5 py-4">Descricao</th><th className="px-5 py-4">Competencia</th><th className="px-5 py-4">Base</th><th className="px-5 py-4">% Aplicada</th><th className="px-5 py-4">Origem</th><th className="px-5 py-4">Comissao</th><th className="px-5 py-4">Status</th><th className="px-5 py-4">Pago em</th><th className="px-5 py-4 text-right">Acoes</th></tr></thead>
              <tbody>
                {rows.length === 0 ? <tr><td colSpan={10} className="px-5 py-10 text-center text-sm text-zinc-500">Nenhuma comissao encontrada com os filtros atuais.</td></tr> : rows.map((item) => {
                  ...
                  return (
                    <tr key={item.id} className="border-b border-zinc-100 align-top">
                      ...
                      <td className="px-5 py-4"><div className="text-sm font-medium text-zinc-900">{item.descricao || "-"}</div><div className="mt-1 text-xs text-zinc-500">{item.observacoes || "Sem observacoes adicionais."}</div></td>
                      <td className="px-5 py-4 text-sm text-zinc-700">{formatDate(item.competencia_data)}</td>
                      <td className="px-5 py-4 text-sm font-medium text-zinc-800">{formatCurrency(item.valor_base)}</td>
                      <td className="px-5 py-4 text-sm font-medium text-zinc-800">{formatPercent(item.percentual_aplicado)}</td>
                      <td className="px-5 py-4"><div className={`inline-flex rounded-full border px-3 py-1 text-xs font-semibold ${origem.badgeClass}`}>{origem.label}</div><div className="mt-2 text-xs text-zinc-500">{origem.description}</div></td>
...
function ResumoCard({ title, value, icon, tone }: { title: string; value: string; icon: ReactNode; tone: "amber" | "emerald" | "sky" | "violet" | "zinc" }) {
  const toneClass = tone === "amber" ? "border-amber-200 bg-amber-50 text-amber-900" : tone === "emerald" ? "border-emerald-200 bg-emerald-50 text-emerald-900" : tone === "sky" ? "border-sky-200 bg-sky-50 text-sky-900" : tone === "violet" ? "border-violet-200 bg-violet-50 text-violet-900" : "border-zinc-200 bg-zinc-50 text-zinc-900";
  return <div className={`rounded-2xl border px-4 py-4 ${toneClass}`}><div className="flex items-center justify-between gap-3"><div className="text-sm font-medium">{title}</div><div>{icon}</div></div><div className="mt-3 text-2xl font-bold">{value}</div></div>;
}

function Field({ label, children }: { label: string; children: ReactNode }) {
  return <div><label className="mb-2 block text-sm font-semibold text-zinc-700">{label}</label>{children}</div>;
}
