...
function buildPermissoesByNivel(nivel: string | null | undefined): Permissoes {
  ...
  (Object.keys(PERMISSIONS) as PermissionKey[]).forEach((key) => {
    resultado[key] = !!nivelNormalizado && PERMISSIONS[key].includes(nivelNormalizado);
  });

  return resultado;
}

function sanitizePermissoesDb(
...
function agendamentosFiltradosBase(lista: AgendamentoFila[], term: string) {
  return lista.filter((item) => {
    const cliente = getJoinedName(item.clientes, "").toLowerCase();
    const servico = getJoinedName(item.servicos as any, "").toLowerCase();
    return cliente.includes(term) || servico.includes(term);
  });
}

function obterTaxaConfigurada(
