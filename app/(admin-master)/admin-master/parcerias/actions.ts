...
export async function criarCampanhaParceria(formData: FormData) {
  ...
  const payload = {
    id_parceiro: idParceiro, nome, descricao: text(formData, "descricao") || null,
    publico: formData.getAll("publico").map(String), locais_exibicao: formData.getAll("locais_exibicao").map(String),
    regioes: { cidade: text(formData, "cidade") || null, uf: text(formData, "uf").toUpperCase().slice(0, 2) || null },
    destino_url: text(formData, "destino_url") || null, cupom_codigo: text(formData, "cupom_codigo") || null,
    modelo_cobranca: text(formData, "modelo_cobranca") || "mensal", valor_contratado: money(text(formData, "valor_contratado")),
    ...
    prioridade: Math.min(100, Math.max(0, intValue(formData, "prioridade", 0))),
    ...
    exclusiva: formData.get("exclusiva") === "on", status: "aguardando_contrato", criado_por: access.usuario.id,
  };
  ...
  await registrarAdminMasterAuditoria({ idAdmin: access.usuario.id, acao: "criar_campanha_parceria", entidade: "parceria_campanhas", entidadeId: data?.id || null, descricao: `Campanha de parceria ${nome} criada.`, payload });
  revalidatePath("/admin-master/parcerias");
}
...
export async function atualizarCampanhaParceria(formData: FormData) {
  ...
  const payload = {
    ...
    limite_frequencia_dia: Math.min(50, Math.max(1, intValue(formData, "limite_frequencia_dia", 2))),
    limite_impressoes_dia: text(formData, "limite_impressoes_dia") ? Math.max(1, intValue(formData, "limite_impressoes_dia", 1)) : null,
    exclusiva: formData.get("exclusiva") === "on",
  };
...
export async function salvarCriativoParceria(formData: FormData) {
  ...
  const payload = { id_campanha: idCampanha, titulo, subtitulo: text(formData, "subtitulo") || null, imagem_url: text(formData, "imagem_url") || null, alt_text: text(formData, "alt_text") || titulo, cta_texto: text(formData, "cta_texto") || "Conhecer oferta", destino_url: text(formData, "destino_url") || null, formato: "card", ordem: 0, ativo: true };
...
export async function gerarContratoParceria(formData: FormData) {
  ...
  const { data: campanha, error: campanhaError } = await supabase.from("parceria_campanhas").select("id,nome,valor_contratado,inicio_em,fim_em,id_parceiro,parceiros_comerciais(razao_social,nome_fantasia,cpf_cnpj,email)").eq("id", idCampanha).single();
