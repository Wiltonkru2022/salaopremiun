...
type AgendamentoInicioRow = {
  ...
  pessoa_atendida_cliente_id: string | null;
  ...
  return runAdminOperation({
    ...
    run: async (supabase) => {
      ...
      const clienteIds = Array.from(
        new Set(
          rows
            .map((item) => item.pessoa_atendida_cliente_id || item.cliente_id)
            .filter(Boolean)
        )
      ) as string[];
      ...
      return rows
        ...
        .map((item) => {
          const clienteOperacionalId =
            item.pessoa_atendida_cliente_id || item.cliente_id;
          return {
            id: item.id,
            data: item.data,
            ...
            cliente_nome: clienteOperacionalId
              ? clientesMap.get(clienteOperacionalId) ?? "Cliente"
              : "Cliente",
