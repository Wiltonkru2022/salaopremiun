...
export async function POST(request: Request) {
  try {
    ...
    if (![15, 30, 60, 120].includes(intervalo)) throw new Error("Intervalo inválido.");
    const diasTrabalho = Array.isArray(body.diasTrabalho) ? body.diasTrabalho : [];
    if (diasTrabalho.length !== 7) throw new Error("Configure os sete dias da semana.");
    ...
    await runAdminOperation({
      ...
      run: async (supabase) => {
        const { error } = await (supabase as any).from("profissionais").update({
        ...
        }).eq("id", context.idProfissional).eq("id_salao", context.idSalao);
    ...
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : "Não foi possível salvar." }, { status: 400, headers: { "Cache-Control": "no-store" } });
  }
}
