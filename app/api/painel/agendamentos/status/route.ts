...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!idAgendamento || !status) {
      return NextResponse.json({ ok: false, error: "Agendamento e status são obrigatórios." }, { status: 400 });
    }
    ...
    if (!appointment) return NextResponse.json({ ok: false, error: "Agendamento não encontrado." }, { status: 404 });
    ...
    if (status === "confirmado" && String(appointment.status || "").toLowerCase() !== "confirmado") {
