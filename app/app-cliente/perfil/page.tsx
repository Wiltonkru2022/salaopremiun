...
function ProfileRow({
  ...
  return (
    <Link
      href={href}
      prefetch={prefetch}
      className={`flex min-h-[4.25rem] items-center justify-between border-b border-zinc-100 px-1 text-base font-bold ${
      ...
      <span className="inline-flex min-w-0 items-center gap-3">
        <span
          className={`relative grid h-10 w-10 shrink-0 place-items-center rounded-xl ${
            gold ? "bg-amber-50 text-[#a66d11]" : "bg-zinc-100 text-zinc-700"
          }`}
        >
          <Icon size={20} />
          {badge && badge > 0 ? (
            <span className="absolute -right-1.5 -top-1.5 inline-flex min-h-5 min-w-5 items-center justify-center rounded-full border-2 border-white bg-[#f5bd42] px-1 text-[10px] font-black leading-none text-black">
              {badge > 99 ? "99+" : badge}
            </span>
          ) : null}
        </span>
...
function SectionTitle({ children }: { children: React.ReactNode }) {
  return (
    <div className="px-1 pb-2 pt-5 text-[11px] font-black uppercase tracking-[0.16em] text-zinc-400">
      {children}
    </div>
  );
}
...
export default async function ClientePerfilPage({
  ...
  const [profile, unreadCount] = await Promise.all([
    getClienteAppProfileData({ idConta: session.idConta }),
    listClienteAppNotifications({
  ...
  return (
    <ClientAppFrame title="Perfil" subtitle="Sua conta no Salão Premium.">
      <section className="mx-auto min-h-dvh max-w-3xl bg-white px-5 pb-28 pt-0 md:px-6">
        <header className="sp-mobile-fixed sticky top-0 z-50 -mx-5 flex items-start justify-between gap-3 border-b border-zinc-100 bg-white/96 px-5 pb-3 pt-[calc(env(safe-area-inset-top)+0.9rem)] backdrop-blur-xl md:-mx-6 md:px-6">
          <div>
            <div className="inline-flex items-center gap-2 text-[11px] font-black uppercase tracking-[0.2em] text-[#9b6110]">
              <Sparkles size={16} />
              Salão Premium Cliente
            </div>
            <h1 className="mt-2 text-[2rem] font-black leading-none tracking-[-0.05em] text-zinc-950">
              Perfil
            </h1>
          </div>
          <div className="flex items-center gap-1">
            <ClientNotificationBell unreadCount={unreadCount} />
            <ClientAppDrawerNav />
          </div>
        </header>

        <div className="mt-6 flex items-center gap-4 rounded-[1.4rem] border border-zinc-100 bg-zinc-50 p-4">
          <div className="relative flex h-20 w-20 shrink-0 items-center justify-center rounded-full bg-zinc-950 text-3xl font-black text-white">
            {initial}
            <span className="absolute -bottom-1 -right-1 grid h-8 w-8 place-items-center rounded-full border-2 border-zinc-50 bg-amber-100 text-[#9b6110]">
              <UserRound size={16} />
            </span>
          </div>
          <div className="min-w-0">
            <h2 className="break-words text-xl font-black tracking-[-0.035em] text-zinc-950">
              {profile.nome || session.nome}
            </h2>
            <p className="mt-1 truncate text-sm font-semibold text-zinc-500">{identity}</p>
            <Link href="/app-cliente/perfil/editar" className="mt-3 inline-flex text-sm font-black text-[#9b6110]">
              Editar meus dados
            </Link>
          </div>
        </div>
        ...
        {profile.creditos.length ? (
          <div className="mt-4 rounded-[1.25rem] border border-amber-100 bg-amber-50 p-4">
            ...
            <div className="mt-3 space-y-2">
              {profile.creditos.map((item) => (
                <div key={item.idSalao} className="flex items-center justify-between rounded-xl bg-white px-3 py-2 text-sm">
                  <span className="min-w-0 truncate font-bold text-zinc-900">{item.salaoNome}</span>
        ...
        <div className="overflow-hidden rounded-[1.35rem] border border-zinc-100 bg-white px-3 shadow-[0_12px_32px_rgba(15,23,42,0.05)]">
          ...
          <ProfileRow href="/app-cliente/notificacoes" label="Notificações" icon={Bell} badge={unreadCount} />
