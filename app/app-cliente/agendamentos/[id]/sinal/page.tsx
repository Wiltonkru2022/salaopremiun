...
export default async function ClienteAgendamentoSinalPage({
  ...
  return (
    <ClientAppFrame
      ...
      subtitle="Pagamento do sinal do seu agendamento."
    >
      <main className="bg-white px-5 pb-32 pt-5 text-zinc-950">
        {salaoId ? <ClientBookingDraftCleanup idSalao={salaoId} /> : null}
        <div className="mx-auto max-w-md">
          <div>
            <h1 className="text-2xl font-black tracking-[-0.04em]">Enviar comprovante</h1>
            <p className="mt-3 text-[1.05rem] leading-7 text-zinc-500">
              Envie o comprovante do pagamento do sinal para o salão confirmar seu horário.
            </p>
          </div>

          <section className="mt-8 rounded-[1.5rem] border border-zinc-200 bg-white p-6 shadow-[0_14px_35px_rgba(15,23,42,0.04)]">
            <div className="grid grid-cols-[56px_1fr] gap-x-4 gap-y-8">
              ...
              <div>
                ...
                <p className="mt-1 text-lg font-black">{servico?.nome || "Serviço"}</p>
                <p className="mt-1 text-sm font-semibold text-zinc-500">Profissional: {profissionalNome}</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <div className="border-l border-zinc-200 pl-6">
                  ...
                  <p className="mt-1 text-lg font-black">{formatCurrency(valorTotal)}</p>
                </div>
              </div>
            </div>
          </section>
          ...
          <section className="mt-5 rounded-[1.5rem] border border-emerald-100 bg-emerald-50/50 p-5">
            <p className="text-sm font-bold uppercase tracking-[0.12em] text-emerald-700">Recebedor</p>
            <p className="mt-2 text-xl font-black">{String(agendamento.sinal_pix_recebedor || "SalaoPremium")}</p>
            <p className="mt-2 break-words text-base font-semibold text-zinc-700">Chave Pix: {String(agendamento.sinal_pix_chave || "")}</p>
          </section>

          <section className="mt-9">
            <h2 className="text-2xl font-black tracking-[-0.04em]">Comprovante de pagamento</h2>
            <p className="mt-3 text-lg leading-7 text-zinc-500">Envie uma imagem ou PDF do comprovante da transferência Pix.</p>
