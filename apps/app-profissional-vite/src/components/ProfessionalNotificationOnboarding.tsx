...
function rememberChoice() {
  try {
    ...
    // O app continua funcionando caso o armazenamento local esteja bloqueado.
  }
}

export function ProfessionalNotificationOnboarding() {
  ...
  async function enable() {
    setLoading(true);
    setMessage("");
    const result = await requestProfessionalPushPermission().catch(() => "ready");
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-end justify-center bg-black/45 p-4 backdrop-blur-sm sm:items-center">
      <section className="relative w-full max-w-md overflow-hidden rounded-[2rem] bg-white p-6 shadow-[0_30px_90px_rgba(0,0,0,0.28)]">
        <button
          type="button"
          onClick={dismiss}
        ...
        <h2 className="mt-4 text-3xl font-black leading-tight tracking-[-0.05em] text-zinc-950">
          Não perca novos agendamentos
        </h2>
        <p className="mt-3 text-base font-semibold leading-7 text-zinc-500">
          Receba confirmações, alterações da agenda e avisos importantes mesmo quando o app estiver fechado.
        </p>
        ...
        <div className="mt-5 rounded-2xl border border-amber-100 bg-amber-50 px-4 py-3 text-sm font-semibold leading-6 text-amber-900">
          Ao tocar em ativar, o Android ou navegador abrirá a permissão oficial de notificações.
        </div>
