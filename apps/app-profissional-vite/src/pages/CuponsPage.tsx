...
export function CuponsPage({ clientes }: { clientes: Cliente[] }) {
  ...
  return <div className="space-y-4">
    <Card className="overflow-hidden border-amber-100 bg-gradient-to-br from-amber-50 to-white">
      <div className="flex items-start justify-between gap-4"><div className="min-w-0"><div className="text-xs font-black uppercase tracking-[0.18em] text-amber-700">Relacionamento</div><h2 className="mt-1 text-2xl font-black tracking-[-0.05em]">Cupons privados</h2><p className="mt-1 text-sm font-bold text-zinc-500">Crie, acompanhe e gerencie campanhas exclusivas pelo WhatsApp.</p></div><div className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-amber-100 text-amber-800"><TicketPercent size={24}/></div></div>
    ...
    <Modal title="Novo cupom" subtitle="Escolha o benefício e quem vai receber." open={open} onClose={() => !loading && setOpen(false)}>
      {!recipients.length ? <div className="space-y-4">
        ...
        <div><div className="mb-2 flex items-center justify-between gap-3"><div className="min-w-0 text-xs font-black uppercase tracking-[0.16em] text-zinc-400">Clientes com WhatsApp</div><button type="button" onClick={()=>setSelected(available.map(c=>c.id))} className="shrink-0 text-xs font-black text-amber-700">Selecionar todas</button></div><div className="relative"><Search size={18} className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-400"/><Input className="pl-10" value={search} onChange={(e)=>setSearch(e.target.value)} placeholder="Buscar cliente"/></div><div className="mt-2 max-h-64 space-y-2 overflow-auto">{available.map((c)=><button type="button" key={c.id} onClick={()=>toggle(c.id)} className={`flex w-full min-w-0 items-center gap-3 rounded-2xl border p-3 text-left ${selected.includes(c.id)?"border-amber-300 bg-amber-50":"border-zinc-200 bg-white"}`}><div className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-zinc-100"><Users size={18}/></div><div className="min-w-0 flex-1"><div className="truncate font-black">{c.nome}</div><div className="truncate text-xs font-bold text-zinc-500">{c.whatsapp || c.telefone}</div></div><span className="shrink-0 text-xs font-black">{selected.includes(c.id)?"Selecionada":""}</span></button>)}</div></div>
...
function DetailTile({label,value}:{label:string;value:string}) { return <div className="min-w-0 rounded-2xl bg-white/10 p-3"><div className="text-[10px] font-black uppercase tracking-wider opacity-60">{label}</div><div className="mt-1 break-words text-lg font-black">{value}</div></div>; }
function Info({label,value}:{label:string;value:string}) { return <div className="min-w-0"><div className="text-[10px] font-black uppercase tracking-wider text-zinc-400">{label}</div><div className="mt-1 break-words font-black text-zinc-950">{value}</div></div>; }
